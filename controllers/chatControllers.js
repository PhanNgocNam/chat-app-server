...
module.exports.createGroupChat = async (req, res) => {
  ...
  try {
    const groupChat = await Chat.create({
      ...
      isGroupChat: true,
      groupAdmin: req.body.current_user_id,
    });
...
//  Delete group chat
//  http://localhost:5000/api/chat/group?chat_id=""
		//  Delete group chat
//  http://localhost:5000/api/chat/group?chat_id=""
module.exports.deleteGroup = async (req, res) => {
  const deleteResult = await Chat.deleteOne({ _id: req.query.chat_id })
  try {
    if (deleteResult) {
      await groupMessage.deleteMany({ room: req.query.chat_id })
